...
interface Trip { id: number; date: string; name: string; start: string; end: string; distance: string; duration: string; color: string }
...
const TripsFilterButton = styled(IconButton)({ backgroundColor: 'rgba(255,255,255,0.07)', borderRadius: '10px', width: 36, height: 36 })
...
const TripColorStripe = styled(Box)<{ stripecolor: string }>(({ stripecolor }) => ({ width: 3, height: 44, borderRadius: '4px', backgroundColor: stripecolor, flexShrink: 0, opacity: 0.7 }))
...
const TripStatsBlock  = styled(Box)({ textAlign: 'right', flexShrink: 0 })
const TripDistance    = styled(Typography)({ fontSize: 14, fontWeight: 700, marginBottom: '3px' })
...
const TripCard = ({ trip, index }: { trip: Trip; index: number }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: cappedDelay, type: 'spring', stiffness: 320, damping: 28 }}>
      <TripCardRow>
  ...
  return (
    <TripsRoot>
      <TripsHeader>
        ...
        <TripsFilterButton><SlidersHorizontal size={16} color="rgba(255,255,255,0.60)" /></TripsFilterButton>
      </TripsHeader>
      <TripListScroll>
        {TRIPS.length === 0 ? (
          <TripEmptyState>
          ...
          Object.entries(grouped).map(([date, trips]) => (
            <TripDateGroup key={date}>
              ...
              {trips.map(trip => <TripCard key={trip.id} trip={trip} index={index++} />)}
            </TripDateGroup>
          ))
        )}
      </TripListScroll>
