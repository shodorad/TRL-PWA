...
const PriceCard         = styled('div')({ ...glassCard, padding: '24px', marginBottom: 16, position: 'relative', overflow: 'hidden' })
const PriceCardGradient = styled('div')({ position: 'absolute', top: -40, right: -40, width: 160, height: 160, background: 'radial-gradient(circle, rgba(200,255,0,0.1) 0%, transparent 70%)', pointerEvents: 'none' })
const PriceRow          = styled('div')({ display: 'flex', alignItems: 'flex-end', gap: 6, marginBottom: 4, position: 'relative' })
const PriceAmount       = styled('span')({ fontSize: 46, fontWeight: 900, color: '#fff', lineHeight: 1, fontFamily: 'Inter, sans-serif', letterSpacing: '-2px' })
const PricePeriod       = styled('span')({ color: 'rgba(255,255,255,0.38)', fontSize: 13, marginBottom: 8, fontFamily: 'Inter, sans-serif' })
const SavingsBadge      = styled('div')({ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 16, padding: '5px 12px', background: 'rgba(74,222,128,0.1)', borderRadius: 99, border: '1px solid rgba(74,222,128,0.2)' })
const SavingsText       = styled('span')({ color: '#4ade80', fontSize: 12.5, fontWeight: 700, fontFamily: 'Inter, sans-serif' })
const Divider           = styled('div')({ height: 1, background: 'rgba(255,255,255,0.07)', margin: '16px 0' })
const FeatureList       = styled('div')({ display: 'flex', flexDirection: 'column', gap: 11 })
const FeatureRow        = styled('div')({ display: 'flex', alignItems: 'center', gap: 11 })
const FeatureIconWrapper = styled('div')({ width: 24, height: 24, borderRadius: 99, background: 'rgba(200,255,0,0.12)', border: '1px solid rgba(200,255,0,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 })
const FeatureText       = styled('span')({ color: 'rgba(255,255,255,0.72)', fontSize: 13, fontFamily: 'Inter, sans-serif' })
  ...
  return (
    <ScreenRoot>
      ...
      <ScrollArea>
          ...
          <PriceCard>
            ...
            {annual && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                <SavingsBadge><SavingsText>You save ${saving}/year vs monthly</SavingsText></SavingsBadge>
              </motion.div>
            )}
            <Divider />
            <FeatureList>
              {features.map(({ icon: Icon, text }) => (
                <FeatureRow key={text}>
                  <FeatureIconWrapper><Icon size={12} color="#C8FF00" /></FeatureIconWrapper>
        ...
        <LegalNote>Your card won't be charged until setup is complete. Cancel any time from settings.</LegalNote>
      </ScrollArea>
