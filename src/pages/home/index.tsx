...
const MapFallbackRoot     = styled(Box)({ width: '100%', height: '100%', backgroundColor: '#0d0d14', display: 'flex', alignItems: 'center', justifyContent: 'center' })
const MapFallbackText     = styled(Typography)({ color: 'rgba(255,255,255,0.3)', fontSize: 12 })
...
const TripBadgeLabel      = styled(Typography)({ color: '#C8FF00', fontSize: 9.5, fontWeight: 700 })
const TripTitle           = styled(Typography)({ fontSize: 14, fontWeight: 700, marginBottom: '2px' })
const TripDestRow         = styled(Box)({ display: 'flex', alignItems: 'center', gap: '5px' })
...
const FloatingHeader = () => {
  ...
  return (
    <MotionHeaderShell initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.15, type: 'spring', stiffness: 300, damping: 28 }}>
      <Box>
        <VehicleRow>
...
const TripInfoCard = () => (
  <MotionTripCardShell initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.28, type: 'spring', stiffness: 300, damping: 28 }}>
    <TripCardContent>
      <TripBadgeRow><TripBadge><Zap size={9} color="#C8FF00" /><TripBadgeLabel>Live Trip</TripBadgeLabel></TripBadge></TripBadgeRow>
      <TripTitle>Morning Commute</TripTitle>
      <TripDestRow><TripDestIcon>AC</TripDestIcon><TripDestText>Candace Ln · 5 mins away</TripDestText></TripDestRow>
    </TripCardContent>
    <MotionGoButton whileTap={{ scale: 0.92 }} variant="contained">GO</MotionGoButton>
  </MotionTripCardShell>
)
